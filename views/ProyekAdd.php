...
loadjs.ready(["wrapper", "head"], function () {
    ...
    fproyekadd.lists.ajuan = <?= $Page->ajuan->toClientList($Page) ?>;
    fproyekadd.lists.status = <?= $Page->status->toClientList($Page) ?>;
...
<span id="el_proyek_ajuan">
    <select
        ...
        data-select2-id="fproyekadd_x_ajuan"
        data-table="proyek"
        data-field="x_ajuan"
...
<script>
loadjs.ready("fproyekadd", function() {
    var options = { name: "x_ajuan", selectId: "fproyekadd_x_ajuan" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fproyekadd.lists.ajuan.lookupOptions.length) {
        options.data = { id: "x_ajuan", form: "fproyekadd" };
    } else {
        options.ajax = { id: "x_ajuan", form: "fproyekadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
loadjs.ready(["fproyekadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fproyekadd", "x_tanggalMulai", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["fproyekadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fproyekadd", "x_tanggalSelesai", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
<?php
    if (in_array("partisipasiproyek", explode(",", $Page->getCurrentDetailTable())) && $partisipasiproyek->DetailAdd) {
?>
...
<?php } ?>
<?php
    if (in_array("ajuanproyek", explode(",", $Page->getCurrentDetailTable())) && $ajuanproyek->DetailAdd) {
?>
<?php if ($Page->getCurrentDetailTable() != "") { ?>
