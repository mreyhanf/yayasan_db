...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fdonasiadd = new ew.Form("fdonasiadd", "add");
    currentPageID = ew.PAGE_ID = "add";
    currentForm = fdonasiadd;

    // Add fields
    var fields = currentTable.fields;
    fdonasiadd.addFields([
    ...
    // Form_CustomValidate
    fdonasiadd.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fdonasiadd.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fdonasiadd.lists.idDonatur = <?= $Page->idDonatur->toClientList($Page) ?>;
    fdonasiadd.lists.idProyek = <?= $Page->idProyek->toClientList($Page) ?>;
    loadjs.done("fdonasiadd");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fdonasiadd" id="fdonasiadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<span id="el_donasi_idDonatur">
    <select
        ...
        data-select2-id="fdonasiadd_x_idDonatur"
        data-table="donasi"
...
<script>
loadjs.ready("fdonasiadd", function() {
    var options = { name: "x_idDonatur", selectId: "fdonasiadd_x_idDonatur" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fdonasiadd.lists.idDonatur.lookupOptions.length) {
        options.data = { id: "x_idDonatur", form: "fdonasiadd" };
    } else {
        options.ajax = { id: "x_idDonatur", form: "fdonasiadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_donasi_idProyek">
    <select
        ...
        data-select2-id="fdonasiadd_x_idProyek"
        data-table="donasi"
...
<script>
loadjs.ready("fdonasiadd", function() {
    var options = { name: "x_idProyek", selectId: "fdonasiadd_x_idProyek" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fdonasiadd.lists.idProyek.lookupOptions.length) {
        options.data = { id: "x_idProyek", form: "fdonasiadd" };
    } else {
        options.ajax = { id: "x_idProyek", form: "fdonasiadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<script>
loadjs.ready(["fdonasiadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fdonasiadd", "x_waktu", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
