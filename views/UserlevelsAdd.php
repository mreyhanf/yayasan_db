...
loadjs.ready(["wrapper", "head"], function () {
    ...
    fuserlevelsadd.lists.userlevelid = <?= $Page->userlevelid->toClientList($Page) ?>;
...
<?php
$onchange = $Page->userlevelid->EditAttrs->prepend("onchange", "");
$onchange = ($onchange) ? ' onchange="' . JsEncode($onchange) . '"' : '';
$Page->userlevelid->EditAttrs["onchange"] = "";
if (IsRTL()) {
    $Page->userlevelid->EditAttrs["dir"] = "rtl";
}
?>
<span id="as_x_userlevelid" class="ew-auto-suggest">
    <input type="<?= $Page->userlevelid->getInputTextType() ?>" class="form-control" name="sv_x_userlevelid" id="sv_x_userlevelid" value="<?= RemoveHtml($Page->userlevelid->EditValue) ?>" size="30" maxlength="11" placeholder="<?= HtmlEncode($Page->userlevelid->getPlaceHolder()) ?>" data-placeholder="<?= HtmlEncode($Page->userlevelid->getPlaceHolder()) ?>"<?= $Page->userlevelid->editAttributes() ?> aria-describedby="x_userlevelid_help">
</span>
<selection-list hidden class="form-control" data-table="userlevels" data-field="x_userlevelid" data-input="sv_x_userlevelid" data-value-separator="<?= $Page->userlevelid->displayValueSeparatorAttribute() ?>" name="x_userlevelid" id="x_userlevelid" value="<?= HtmlEncode($Page->userlevelid->CurrentValue) ?>"<?= $onchange ?>></selection-list>
...
loadjs.ready("fuserlevelsadd", function() {
    fuserlevelsadd.createAutoSuggest(Object.assign({"id":"x_userlevelid","forceSelect":false}, ew.vars.tables.userlevels.fields.userlevelid.autoSuggestOptions));
});
</script>
