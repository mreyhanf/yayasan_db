...
// Page object
$KontaktargetproposalEdit = &$Page;
?>
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { kontaktargetproposal: currentTable } });
var currentForm, currentPageID;
var fkontaktargetproposaledit;
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fkontaktargetproposaledit = new ew.Form("fkontaktargetproposaledit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fkontaktargetproposaledit;

    // Add fields
    var fields = currentTable.fields;
    fkontaktargetproposaledit.addFields([
    ...
    // Form_CustomValidate
    fkontaktargetproposaledit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fkontaktargetproposaledit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fkontaktargetproposaledit.lists.idTargetProposal = <?= $Page->idTargetProposal->toClientList($Page) ?>;
    loadjs.done("fkontaktargetproposaledit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fkontaktargetproposaledit" id="fkontaktargetproposaledit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<input type="hidden" data-table="kontaktargetproposal" data-field="x_kontak" data-hidden="1" name="o_kontak" id="o_kontak" value="<?= HtmlEncode($Page->kontak->OldValue ?? $Page->kontak->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<span id="el_kontaktargetproposal_idTargetProposal">
    <select
        ...
        data-select2-id="fkontaktargetproposaledit_x_idTargetProposal"
...
<script>
loadjs.ready("fkontaktargetproposaledit", function() {
    var options = { name: "x_idTargetProposal", selectId: "fkontaktargetproposaledit_x_idTargetProposal" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fkontaktargetproposaledit.lists.idTargetProposal.lookupOptions.length) {
        options.data = { id: "x_idTargetProposal", form: "fkontaktargetproposaledit" };
    } else {
        options.ajax = { id: "x_idTargetProposal", form: "fkontaktargetproposaledit", limit: ew.LOOKUP_PAGE_SIZE };
    }
