...
loadjs.ready(["wrapper", "head"], function () {
    var $ = jQuery;
    // Form object
    fkegiatanedit = new ew.Form("fkegiatanedit", "edit");
    currentPageID = ew.PAGE_ID = "edit";
    currentForm = fkegiatanedit;

    // Add fields
    var fields = currentTable.fields;
    fkegiatanedit.addFields([
    ...
    // Form_CustomValidate
    fkegiatanedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fkegiatanedit.validateRequired = ew.CLIENT_VALIDATE;

    // Dynamic selection lists
    fkegiatanedit.lists.penanggungJawab = <?= $Page->penanggungJawab->toClientList($Page) ?>;
    fkegiatanedit.lists.status = <?= $Page->status->toClientList($Page) ?>;
    loadjs.done("fkegiatanedit");
});
</script>
<script>
...
$Page->showMessage();
?>
<form name="fkegiatanedit" id="fkegiatanedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post">
...
<?php if ($Page->idKegiatan->Visible) { // idKegiatan ?>
    <div id="r_idKegiatan"<?= $Page->idKegiatan->rowAttributes() ?>>
        <label id="elh_kegiatan_idKegiatan" class="<?= $Page->LeftColumnClass ?>"><?= $Page->idKegiatan->caption() ?><?= $Page->idKegiatan->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->idKegiatan->getDisplayValue($Page->idKegiatan->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="kegiatan" data-field="x_idKegiatan" data-hidden="1" name="x_idKegiatan" id="x_idKegiatan" value="<?= HtmlEncode($Page->idKegiatan->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<span id="el_kegiatan_penanggungJawab">
    <select
        ...
        data-select2-id="fkegiatanedit_x_penanggungJawab"
        data-table="kegiatan"
...
<script>
loadjs.ready("fkegiatanedit", function() {
    var options = { name: "x_penanggungJawab", selectId: "fkegiatanedit_x_penanggungJawab" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fkegiatanedit.lists.penanggungJawab.lookupOptions.length) {
        options.data = { id: "x_penanggungJawab", form: "fkegiatanedit" };
    } else {
        options.ajax = { id: "x_penanggungJawab", form: "fkegiatanedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script>
loadjs.ready(["fkegiatanedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fkegiatanedit", "x_tanggalMulai", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["fkegiatanedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fkegiatanedit", "x_tanggalSelesai", jQuery.extend(true, {"useCurrent":false}, options));
});
</script>
<?php } ?>
</span>
...
<?php
    if (in_array("partisipasikegiatan", explode(",", $Page->getCurrentDetailTable())) && $partisipasikegiatan->DetailEdit) {
?>
