 ...
 * Page class
 */
class AjuanproyekView extends Ajuanproyek
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "AjuanproyekView";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("idAjuanProyek") ?? Route("idAjuanProyek")) !== null) {
            $this->RecKey["idAjuanProyek"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("idAjuanProyek") ?? Route("idAjuanProyek")) !== null) {
                ...
                $this->RecKey["idAjuanProyek"] = $this->idAjuanProyek->QueryStringValue;
            } elseif (Post("idAjuanProyek") !== null) {
                $this->idAjuanProyek->setFormValue(Post("idAjuanProyek"));
                $this->RecKey["idAjuanProyek"] = $this->idAjuanProyek->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->idAjuanProyek->setQueryStringValue($keyValue);
                $this->RecKey["idAjuanProyek"] = $this->idAjuanProyek->QueryStringValue;
            } else {
                $returnUrl = "AjuanproyekList"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                        ...
                        if (!$res) { // Load record based on key
                            ...
                            $returnUrl = "AjuanproyekList"; // No matching record, return to list
                        }
                    break;
            }
        } else {
            $returnUrl = "AjuanproyekList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
    ...
    protected function setupOtherOptions()
    {
        ...
        $item = &$option->add("detail_kontakajuanproyek");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->TablePhrase("kontakajuanproyek", "TblCaption");
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("KontakajuanproyekList?" . Config("TABLE_SHOW_MASTER") . "=ajuanproyek&" . GetForeignKeyUrl("fk_idAjuanProyek", $this->idAjuanProyek->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        $detailPageObj = Container("KontakajuanproyekGrid");
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'ajuanproyek')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=kontakajuanproyek"))) . "\">" . $Language->phrase("MasterDetailViewLink", null) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
            $detailViewTblVar .= "kontakajuanproyek";
        }
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'ajuanproyek')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailEditLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=kontakajuanproyek"))) . "\">" . $Language->phrase("MasterDetailEditLink", null) . "</a></li>";
            if ($detailEditTblVar != "") {
                $detailEditTblVar .= ",";
            }
            $detailEditTblVar .= "kontakajuanproyek";
        }
        if ($detailPageObj->DetailAdd && $Security->canAdd() && $Security->allowAdd(CurrentProjectID() . 'ajuanproyek')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-copy\" data-action=\"add\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailCopyLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=kontakajuanproyek"))) . "\">" . $Language->phrase("MasterDetailCopyLink", null) . "</a></li>";
            if ($detailCopyTblVar != "") {
                $detailCopyTblVar .= ",";
            }
