 ...
 * Page class
 */
class DonasiGrid extends Donasi
{
    use MessagesTrait;
    ...
    // Table name
    public $TableName = 'donasi';

    // Page object name
    public $PageObjName = "DonasiGrid";
    ...
    public function __construct()
    {
        ...
        // Table object (donasi)
        if (!isset($GLOBALS["donasi"]) || get_class($GLOBALS["donasi"]) == PROJECT_NAMESPACE . "donasi") {
            $GLOBALS["donasi"] = &$this;
        }
        $this->AddUrl = "DonasiAdd";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'donasi');
        }
    ...
    public function run()
    {
        ...
        $this->idDonasi->setVisibility();
        $this->idDonatur->setVisibility();
        ...
        // Set up lookup cache
        $this->setupLookupOptions($this->idDonatur);
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "donatur") {
            $masterTbl = Container("donatur");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetchAssociative();
            $this->MasterRecordExists = count($rsmaster) > 0;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("DonaturList"); // Return to master page
                return;
            } else {
        ...
        for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
            ...
            if (!$this->emptyRow()) {
                ...
                if ($gridInsert) {
                    ...
                    $key .= $this->idDonasi->CurrentValue;
    ...
    {
        global $CurrentForm;
        if ($CurrentForm->hasValue("x_idDonatur") && $CurrentForm->hasValue("o_idDonatur") && $this->idDonatur->CurrentValue != $this->idDonatur->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_nominal") && $CurrentForm->hasValue("o_nominal") && $this->nominal->CurrentValue != $this->nominal->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_idProyek") && $CurrentForm->hasValue("o_idProyek") && $this->idProyek->CurrentValue != $this->idProyek->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_waktu") && $CurrentForm->hasValue("o_waktu") && $this->waktu->CurrentValue != $this->waktu->OldValue) {
            return false;
        }
        return true;
    }
    ...
    {
        $this->idDonasi->clearErrorMessage();
        $this->idDonatur->clearErrorMessage();
        $this->nominal->clearErrorMessage();
        $this->idProyek->clearErrorMessage();
        $this->waktu->clearErrorMessage();
    }
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetall") {
                ...
                $this->DbMasterFilter = "";
                $this->DbDetailFilter = "";
                        $this->idDonatur->setSessionValue("");
            }
    ...
    {
        $this->idDonasi->CurrentValue = null;
        $this->idDonasi->OldValue = $this->idDonasi->CurrentValue;
        $this->idDonatur->CurrentValue = null;
        $this->idDonatur->OldValue = $this->idDonatur->CurrentValue;
        $this->nominal->CurrentValue = null;
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'idDonasi' first before field var 'x_idDonasi'
        $val = $CurrentForm->hasValue("idDonasi") ? $CurrentForm->getValue("idDonasi") : $CurrentForm->getValue("x_idDonasi");
        if (!$this->idDonasi->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
            $this->idDonasi->setFormValue($val);
        }

        // Check field name 'idDonatur' first before field var 'x_idDonatur'
        $val = $CurrentForm->hasValue("idDonatur") ? $CurrentForm->getValue("idDonatur") : $CurrentForm->getValue("x_idDonatur");
        if (!$this->idDonatur->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->idDonatur->Visible = false; // Disable update for API request
            } else {
                $this->idDonatur->setFormValue($val);
            }
        }
        if ($CurrentForm->hasValue("o_idDonatur")) {
            $this->idDonatur->setOldValue($CurrentForm->getValue("o_idDonatur"));
        }

        // Check field name 'nominal' first before field var 'x_nominal'
        $val = $CurrentForm->hasValue("nominal") ? $CurrentForm->getValue("nominal") : $CurrentForm->getValue("x_nominal");
        if (!$this->nominal->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->nominal->Visible = false; // Disable update for API request
            } else {
                $this->nominal->setFormValue($val, true, $validate);
            }
        }
        if ($CurrentForm->hasValue("o_nominal")) {
            $this->nominal->setOldValue($CurrentForm->getValue("o_nominal"));
        }
        ...
        if ($CurrentForm->hasValue("o_waktu")) {
            $this->waktu->setOldValue($CurrentForm->getValue("o_waktu"));
        }
    }
    ...
    public function restoreFormValues()
    {
        ...
        if (!$this->isGridAdd() && !$this->isAdd()) {
            $this->idDonasi->CurrentValue = $this->idDonasi->FormValue;
        }
        $this->idDonatur->CurrentValue = $this->idDonatur->FormValue;
    ...
    protected function newRow()
    {
        ...
        $row = [];
        $row['idDonasi'] = $this->idDonasi->CurrentValue;
        $row['idDonatur'] = $this->idDonatur->CurrentValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($this->idDonatur->getSessionValue() != "") {
                ...
                $curVal = trim(strval($this->idDonatur->CurrentValue));
                if ($curVal != "") {
                    $this->idDonatur->ViewValue = $this->idDonatur->lookupCacheOption($curVal);
                } else {
                    $this->idDonatur->ViewValue = $this->idDonatur->Lookup !== null && is_array($this->idDonatur->lookupOptions()) ? $curVal : null;
                }
                if ($this->idDonatur->ViewValue !== null) { // Load from cache
                    $this->idDonatur->EditValue = array_values($this->idDonatur->lookupOptions());
                } else { // Lookup from database
                    if ($curVal == "") {
                        $filterWrk = "0=1";
                    } else {
                        $filterWrk = "`idDonatur`" . SearchString("=", $this->idDonatur->CurrentValue, DATATYPE_NUMBER, "");
                    }
                    $sqlWrk = $this->idDonatur->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                    $conn = Conn();
                    $config = $conn->getConfiguration();
                    $config->setResultCacheImpl($this->Cache);
                    $rswrk = $conn->executeCacheQuery($sqlWrk, [], [], $this->CacheProfile)->fetchAll();
                    $ari = count($rswrk);
                    $arwrk = $rswrk;
                    $this->idDonatur->EditValue = $arwrk;
                }
            ...
            $this->nominal->EditValue = HtmlEncode($this->nominal->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
        if ($this->idDonasi->Required) {
            if (!$this->idDonasi->IsDetailKey && EmptyValue($this->idDonasi->FormValue)) {
                $this->idDonasi->addErrorMessage(str_replace("%s", $this->idDonasi->caption(), $this->idDonasi->RequiredErrorMessage));
            }
        }
        if ($this->idDonatur->Required) {
            if (!$this->idDonatur->IsDetailKey && EmptyValue($this->idDonatur->FormValue)) {
                $this->idDonatur->addErrorMessage(str_replace("%s", $this->idDonatur->caption(), $this->idDonatur->RequiredErrorMessage));
            }
        }
        if ($this->nominal->Required) {
            if (!$this->nominal->IsDetailKey && EmptyValue($this->nominal->FormValue)) {
                $this->nominal->addErrorMessage(str_replace("%s", $this->nominal->caption(), $this->nominal->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->nominal->FormValue)) {
            $this->nominal->addErrorMessage($this->nominal->getErrorMessage(false));
        }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            // idDonatur
            if ($this->idDonatur->getSessionValue() != "") {
                $this->idDonatur->ReadOnly = true;
            }
            $this->idDonatur->setDbValueDef($rsnew, $this->idDonatur->CurrentValue, null, $this->idDonatur->ReadOnly);

            // nominal
            $this->nominal->setDbValueDef($rsnew, $this->nominal->CurrentValue, 0, $this->nominal->ReadOnly);

            // idProyek
            $this->idProyek->setDbValueDef($rsnew, $this->idProyek->CurrentValue, null, $this->idProyek->ReadOnly);

            // waktu
            $this->waktu->setDbValueDef($rsnew, UnFormatDateTime($this->waktu->CurrentValue, $this->waktu->formatPattern()), CurrentDate(), $this->waktu->ReadOnly);

            // Check referential integrity for master table 'donatur'
            $detailKeys = [];
            $keyValue = $rsnew['idDonatur'] ?? $rsold['idDonatur'];
            $detailKeys['idDonatur'] = $keyValue;
            $masterTable = Container("donatur");
            ...
            if (!$validMasterRecord) {
                $relatedRecordMsg = str_replace("%t", "donatur", $Language->phrase("RelatedRecordRequired"));
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->getCurrentMasterTable() == "donatur") {
            $this->idDonatur->CurrentValue = $this->idDonatur->getSessionValue();
        }

        // Check referential integrity for master table 'donasi'
        $validMasterRecord = true;
        $detailKeys = [];
        $detailKeys["idDonatur"] = $this->idDonatur->CurrentValue;
        $masterTable = Container("donatur");
        $masterFilter = $this->getMasterFilter($masterTable, $detailKeys);
        if (!EmptyValue($masterFilter)) {
            $rsmaster = $masterTable->loadRs($masterFilter)->fetch();
        ...
        // idDonatur
        $this->idDonatur->setDbValueDef($rsnew, $this->idDonatur->CurrentValue, null, false);

        // nominal
        $this->nominal->setDbValueDef($rsnew, $this->nominal->CurrentValue, 0, false);

        // idProyek
        $this->idProyek->setDbValueDef($rsnew, $this->idProyek->CurrentValue, null, false);

        // waktu
        $this->waktu->setDbValueDef($rsnew, UnFormatDateTime($this->waktu->CurrentValue, $this->waktu->formatPattern()), CurrentDate(), false);
    ...
    protected function setupMasterParms()
    {
        ...
        if ($masterTblVar == "donatur") {
            $masterTbl = Container("donatur");
            $this->idDonatur->Visible = false;
