 ...
 * Page class
 */
class ProyekGrid extends Proyek
{
    use MessagesTrait;
    ...
    {
        global $CurrentForm;
        if ($CurrentForm->hasValue("x_ajuan") && $CurrentForm->hasValue("o_ajuan") && $this->ajuan->CurrentValue != $this->ajuan->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_biayaTerkumpul") && $CurrentForm->hasValue("o_biayaTerkumpul") && $this->biayaTerkumpul->CurrentValue != $this->biayaTerkumpul->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_tanggalMulai") && $CurrentForm->hasValue("o_tanggalMulai") && $this->tanggalMulai->CurrentValue != $this->tanggalMulai->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_tanggalSelesai") && $CurrentForm->hasValue("o_tanggalSelesai") && $this->tanggalSelesai->CurrentValue != $this->tanggalSelesai->OldValue) {
            return false;
        }
    ...
    public function resetFormError()
    {
        ...
        $this->ajuan->clearErrorMessage();
        $this->biayaTerkumpul->clearErrorMessage();
        $this->tanggalMulai->clearErrorMessage();
        $this->tanggalSelesai->clearErrorMessage();
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->idProyek->IsDetailKey && !$this->isGridAdd() && !$this->isAdd()) {
        ...
        if ($CurrentForm->hasValue("o_ajuan")) {
            $this->ajuan->setOldValue($CurrentForm->getValue("o_ajuan"));
        }
        ...
        if ($CurrentForm->hasValue("o_biayaTerkumpul")) {
            $this->biayaTerkumpul->setOldValue($CurrentForm->getValue("o_biayaTerkumpul"));
        }
        ...
        if ($CurrentForm->hasValue("o_tanggalMulai")) {
            $this->tanggalMulai->setOldValue($CurrentForm->getValue("o_tanggalMulai"));
        }
        ...
        if ($CurrentForm->hasValue("o_tanggalSelesai")) {
            $this->tanggalSelesai->setOldValue($CurrentForm->getValue("o_tanggalSelesai"));
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($this->ajuan->getSessionValue() != "") {
                $this->ajuan->CurrentValue = GetForeignKeyValue($this->ajuan->getSessionValue());
                $this->ajuan->OldValue = $this->ajuan->CurrentValue;
                $curVal = strval($this->ajuan->CurrentValue);
                if ($curVal != "") {
                    $this->ajuan->ViewValue = $this->ajuan->lookupCacheOption($curVal);
            ...
            if (strval($this->biayaTerkumpul->EditValue) != "" && is_numeric($this->biayaTerkumpul->EditValue)) {
                ...
                $this->biayaTerkumpul->OldValue = $this->biayaTerkumpul->EditValue;
            }
    ...
    protected function editRow()
    {
        ...
        if (!$rsold) {
            ...
            $detailKeys = [];
            $keyValue = $rsnew['ajuan'] ?? $rsold['ajuan'];
            $detailKeys['ajuan'] = $keyValue;
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->getCurrentMasterTable() == "ajuanproyek") {
            $this->ajuan->CurrentValue = $this->ajuan->getSessionValue();
        }

        // Check referential integrity for master table 'proyek'
        $validMasterRecord = true;
        $detailKeys = [];
        $detailKeys["ajuan"] = $this->ajuan->CurrentValue;
        $masterTable = Container("ajuanproyek");
        $masterFilter = $this->getMasterFilter($masterTable, $detailKeys);
        if (!EmptyValue($masterFilter)) {
            $rsmaster = $masterTable->loadRs($masterFilter)->fetch();
