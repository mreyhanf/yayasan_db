 ...
 * Page class
 */
class DonasiEdit extends Donasi
{
    use MessagesTrait;
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("idDonasi") ?? Key(0) ?? Route(2)) !== null) {
                $this->idDonasi->setQueryStringValue($keyValue);
                $this->idDonasi->setOldValue($this->idDonasi->QueryStringValue);
            } elseif (Post("idDonasi") !== null) {
                $this->idDonasi->setFormValue(Post("idDonasi"));
                $this->idDonasi->setOldValue($this->idDonasi->FormValue);
            } else {
            ...
            if (Post("action") !== null) {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("idDonasi") ?? Route("idDonasi")) !== null) {
                    $this->idDonasi->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("DonasiList"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "DonasiList") {
    ...
    public function renderRow()
    {
        ...
        // idDonasi
        $this->idDonasi->RowCssClass = "row";

        // idDonatur
        $this->idDonatur->RowCssClass = "row";

        // nominal
        $this->nominal->RowCssClass = "row";
    ...
    protected function setupMasterParms()
    {
        ...
        if (($master = Get(Config("TABLE_SHOW_MASTER"), Get(Config("TABLE_MASTER")))) !== null) {
            ...
            if ($masterTblVar == "donatur") {
                ...
                if (($parm = Get("fk_idDonatur", Get("idDonatur"))) !== null) {
                    $masterTbl->idDonatur->setQueryStringValue($parm);
                    $this->idDonatur->setQueryStringValue($masterTbl->idDonatur->QueryStringValue);
                    $this->idDonatur->setSessionValue($this->idDonatur->QueryStringValue);
                    if (!is_numeric($masterTbl->idDonatur->QueryStringValue)) {
                        $validMaster = false;
                    }
                } else {
                    $validMaster = false;
            ...
            if ($masterTblVar == "donatur") {
                $validMaster = true;
                $masterTbl = Container("donatur");
                if (($parm = Post("fk_idDonatur", Post("idDonatur"))) !== null) {
                    $masterTbl->idDonatur->setFormValue($parm);
                    $this->idDonatur->setFormValue($masterTbl->idDonatur->FormValue);
                    $this->idDonatur->setSessionValue($this->idDonatur->FormValue);
                    if (!is_numeric($masterTbl->idDonatur->FormValue)) {
                        $validMaster = false;
                    }
                } else {
                    $validMaster = false;
        ...
        if ($validMaster) {
            ...
            if ($masterTblVar != "donatur") {
                if ($this->idDonatur->CurrentValue == "") {
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("DonasiList"), "", $this->TableVar, true);
        $pageId = "edit";
