 ...
 * Page class
 */
class ProyekList extends Proyek
{
    use MessagesTrait;
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "ProyekAdd?" . Config("TABLE_SHOW_DETAIL") . "=";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "ProyekDelete";
        $this->MultiUpdateUrl = "ProyekUpdate";
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->idProyek); // idProyek
            $this->updateSort($this->ajuan); // ajuan
            $this->updateSort($this->biayaTerkumpul); // biayaTerkumpul
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                $orderBy = "";
                $this->setSessionOrderBy($orderBy);
                $this->idProyek->setSort("");
                $this->ajuan->setSort("");
                $this->biayaTerkumpul->setSort("");
    ...
    protected function setupListOptions()
    {
        ...
        // "detail_partisipasiproyek"
        $item = &$this->ListOptions->add("detail_partisipasiproyek");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'partisipasiproyek');
        $item->OnLeft = false;
        $item->ShowInButtonGroup = false;

        // "detail_ajuanproyek"
        $item = &$this->ListOptions->add("detail_ajuanproyek");
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'ajuanproyek');
        $item->OnLeft = false;
        $item->ShowInButtonGroup = false;
        ...
        // Set up detail pages
        $pages = new SubPages();
        $pages->add("partisipasiproyek");
        $pages->add("ajuanproyek");
    ...
    public function renderListOptions()
    {
        ...
        if ($opt && !$this->isExport() && !$this->CurrentAction) {
            ...
            foreach ($this->ListActions->Items as $listaction) {
                ...
                if ($listaction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    $link = "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fproyeklist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button></li>";
                    if ($link != "") {
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" data-ew-action=\"submit\" form=\"fproyeklist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listaction->toDataAttrs() . ">" . $icon . $listaction->Caption . "</button>";
                        }
                    }
                }
            }
        ...
        // "detail_partisipasiproyek"
        $opt = $this->ListOptions["detail_partisipasiproyek"];
        if ($Security->allowList(CurrentProjectID() . 'partisipasiproyek')) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("partisipasiproyek", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("PartisipasiproyekList?" . Config("TABLE_SHOW_MASTER") . "=proyek&" . GetForeignKeyUrl("fk_idProyek", $this->idProyek->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("PartisipasiproyekGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'proyek')) {
                $caption = $Language->phrase("MasterDetailViewLink", null);
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=partisipasiproyek");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
                $detailViewTblVar .= "partisipasiproyek";
            }
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'proyek')) {
                $caption = $Language->phrase("MasterDetailEditLink", null);
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=partisipasiproyek");
                ...
                $detailEditTblVar .= "partisipasiproyek";
            }
            if ($detailPage->DetailAdd && $Security->canAdd() && $Security->allowAdd(CurrentProjectID() . 'proyek')) {
                $caption = $Language->phrase("MasterDetailCopyLink", null);
                $url = $this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=partisipasiproyek");
                ...
                $detailCopyTblVar .= "partisipasiproyek";
            }
            if ($links != "") {
        ...
        // "detail_ajuanproyek"
        $opt = $this->ListOptions["detail_ajuanproyek"];
        if ($Security->allowList(CurrentProjectID() . 'ajuanproyek')) {
            $body = $Language->phrase("DetailLink") . $Language->TablePhrase("ajuanproyek", "TblCaption");
            $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode("AjuanproyekList?" . Config("TABLE_SHOW_MASTER") . "=proyek&" . GetForeignKeyUrl("fk_ajuan", $this->ajuan->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
            $detailPage = Container("AjuanproyekGrid");
            if ($detailPage->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'proyek')) {
                $caption = $Language->phrase("MasterDetailViewLink", null);
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=ajuanproyek");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
                $detailViewTblVar .= "ajuanproyek";
            }
            if ($detailPage->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'proyek')) {
                $caption = $Language->phrase("MasterDetailEditLink", null);
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=ajuanproyek");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailEditTblVar != "") {
                    $detailEditTblVar .= ",";
                }
                $detailEditTblVar .= "ajuanproyek";
            }
            if ($detailPage->DetailAdd && $Security->canAdd() && $Security->allowAdd(CurrentProjectID() . 'proyek')) {
                $caption = $Language->phrase("MasterDetailCopyLink", null);
                $url = $this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=ajuanproyek");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-copy\" data-action=\"add\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailCopyTblVar != "") {
                    $detailCopyTblVar .= ",";
                }
                $detailCopyTblVar .= "ajuanproyek";
            }
            if ($links != "") {
                $body .= "<button class=\"dropdown-toggle btn btn-default ew-detail\" data-bs-toggle=\"dropdown\"></button>";
                $body .= "<ul class=\"dropdown-menu\">" . $links . "</ul>";
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->idProyek->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        $detailTableLink = "";
                $item = &$option->add("detailadd_partisipasiproyek");
                $url = $this->getAddUrl(Config("TABLE_SHOW_DETAIL") . "=partisipasiproyek");
                $detailPage = Container("PartisipasiproyekGrid");
                $caption = $Language->phrase("Add") . "&nbsp;" . $this->tableCaption() . "/" . $detailPage->tableCaption();
                $item->Body = "<a class=\"ew-detail-add-group ew-detail-add\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode(GetUrl($url)) . "\">" . $caption . "</a>";
                $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'proyek') && $Security->canAdd());
                if ($item->Visible) {
                    if ($detailTableLink != "") {
                        $detailTableLink .= ",";
                    }
                    $detailTableLink .= "partisipasiproyek";
                }
                $item = &$option->add("detailadd_ajuanproyek");
                $url = $this->getAddUrl(Config("TABLE_SHOW_DETAIL") . "=ajuanproyek");
                $detailPage = Container("AjuanproyekGrid");
                $caption = $Language->phrase("Add") . "&nbsp;" . $this->tableCaption() . "/" . $detailPage->tableCaption();
                $item->Body = "<a class=\"ew-detail-add-group ew-detail-add\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode(GetUrl($url)) . "\">" . $caption . "</a>";
                $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'proyek') && $Security->canAdd());
        ...
        if ($this->UseColumnVisibility) {
            ...
            $option->add("idProyek", $this->createColumnOption("idProyek"));
            $option->add("ajuan", $this->createColumnOption("ajuan"));
            $option->add("biayaTerkumpul", $this->createColumnOption("biayaTerkumpul"));
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fproyeksrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fproyeksrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions->Items as $listaction) {
            if ($listaction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fproyeklist"' . $listaction->toDataAttrs() . '>' . $icon . '</button>';
