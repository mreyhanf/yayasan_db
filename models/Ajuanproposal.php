...
/**
 * Table class for ajuanproposal
 */
class Ajuanproposal extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'ajuanproposal';
        $this->TableName = 'ajuanproposal';
        $this->TableType = 'TABLE';

        // Update Table
        $this->UpdateTable = "`ajuanproposal`";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->status->Lookup = new Lookup('status', 'ajuanproposal', false, '', ["","","",""], [], [], [], [], [], [], '', '', "");
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "`ajuanproposal`";
    }
    ...
    {
        return "`idProyek` = @idProyek@ AND `idTargetProposal` = @idTargetProposal@";
    }
    ...
    public function setKey($key, $current = false)
    {
        ...
        if (count($keys) == 2) {
            ...
            if ($current) {
                $this->idTargetProposal->CurrentValue = $keys[1];
            } else {
                $this->idTargetProposal->OldValue = $keys[1];
            }
        }
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("AjuanproposalList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        } elseif ($pageName == "AjuanproposalEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "AjuanproposalAdd") {
    ...
    public function getApiPageName($action)
    {
        switch (strtolower($action)) {
            case Config("API_VIEW_ACTION"):
                return "AjuanproposalView";
            case Config("API_ADD_ACTION"):
                return "AjuanproposalAdd";
            case Config("API_EDIT_ACTION"):
                return "AjuanproposalEdit";
            case Config("API_DELETE_ACTION"):
                return "AjuanproposalDelete";
            case Config("API_LIST_ACTION"):
                return "AjuanproposalList";
            default:
                return "";
        }
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("AjuanproposalView", $this->getUrlParm($parm));
        } else {
            $url = $this->keyUrl("AjuanproposalView", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
        }
    ...
    {
        if ($parm != "") {
            $url = "AjuanproposalAdd?" . $this->getUrlParm($parm);
        } else {
            $url = "AjuanproposalAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("AjuanproposalEdit", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("AjuanproposalAdd", $this->getUrlParm($parm));
        return $this->addMasterUrl($url);
    }
    ...
    {
        return $this->keyUrl("AjuanproposalDelete", $this->getUrlParm());
    }
    ...
    public function keyUrl($url, $parm = "")
    {
        ...
        if ($this->idTargetProposal->CurrentValue !== null) {
            $url .= $this->RouteCompositeKeySeparator . $this->encodeKeyValue($this->idTargetProposal->CurrentValue);
        } else {
            return "javascript:ew.alert(ew.language.phrase('InvalidRecord'));";
        }
        if ($parm != "") {
    ...
    public function getRecordKeys()
    {
        ...
        if (is_array($arKeys)) {
            foreach ($arKeys as $key) {
                ...
                if (!is_numeric($key[0])) { // idProyek
                    continue;
                }
                if (!is_numeric($key[1])) { // idTargetProposal
                    continue;
                }
                $ar[] = $key;
            }
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->idProyek->CurrentValue = $key[0];
            } else {
                $this->idProyek->OldValue = $key[0];
            }
            if ($setCurrent) {
                $this->idTargetProposal->CurrentValue = $key[1];
            } else {
                $this->idTargetProposal->OldValue = $key[1];
            }
