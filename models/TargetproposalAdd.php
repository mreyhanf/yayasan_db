 ...
 * Page class
 */
class TargetproposalAdd extends Targetproposal
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "TargetproposalAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("TargetproposalList"); // No matching record, return to list
                    return;
                }
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "TargetproposalList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "TargetproposalView") {
    ...
    protected function validateForm()
    {
        ...
        if (in_array("kontaktargetproposal", $detailTblVar) && $detailPage->DetailAdd) {
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($addRow) {
            ...
            $detailPage = Container("KontaktargetproposalGrid");
            if (in_array("kontaktargetproposal", $detailTblVar) && $detailPage->DetailAdd) {
                $detailPage->idTargetProposal->setSessionValue($this->idTargetProposal->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "kontaktargetproposal"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->idTargetProposal->setSessionValue(""); // Clear master key if insert failed
                }
            }
        }
