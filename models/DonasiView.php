 ...
 * Page class
 */
class DonasiView extends Donasi
{
    use MessagesTrait;
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("idDonasi") ?? Route("idDonasi")) !== null) {
                ...
                $this->RecKey["idDonasi"] = $this->idDonasi->QueryStringValue;
            } elseif (Post("idDonasi") !== null) {
                $this->idDonasi->setFormValue(Post("idDonasi"));
                $this->RecKey["idDonasi"] = $this->idDonasi->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->idDonasi->setQueryStringValue($keyValue);
                $this->RecKey["idDonasi"] = $this->idDonasi->QueryStringValue;
            } else {
                $returnUrl = "DonasiList"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                        ...
                        if (!$res) { // Load record based on key
                            ...
                            $returnUrl = "DonasiList"; // No matching record, return to list
                        }
                    break;
            }
        } else {
            $returnUrl = "DonasiList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
